...
function updateInputLabels() {
  ...
  const input3Container = document.querySelector('label[for="input3"]').parentNode;
  const input4Container = document.querySelector('label[for="input4"]').parentNode;
  
  if (!label1 || !label2 || !label3 || !label4 || !input3Container || !input4Container) return;
  
  switch (selectedShape) {
    case 'bendera': // Huruf X
...
function updateCanvasCursor() {
  canvas2d.classList.remove('canvas-translate', 'canvas-rotate', 'canvas-scale');
